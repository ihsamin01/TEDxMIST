...
export default function RegisterForm() {
  ...
  const error = (key: keyof typeof BLANK) => state.errors[key];
    ...
    <form action={formAction} noValidate className="space-y-12">
      ...
      <fieldset className="grid gap-6 sm:grid-cols-2" disabled={pending}>
        ...
        {registration.paymentNumber && (
          <p className="-mt-2 rounded-xl border border-line bg-ink-soft px-5 py-4 text-sm leading-relaxed text-white/75 sm:col-span-2">
            Send{" "}
            <strong className="font-bold text-white">
              {registration.currency} {registration.fee}
            </strong>{" "}
            to{" "}
            <strong className="font-bold text-ted">
              {registration.paymentNumber}
            </strong>{" "}
            first, then enter the transaction ID from the confirmation message
            below. Your seat is confirmed once we have verified the payment.
          </p>
        )}
